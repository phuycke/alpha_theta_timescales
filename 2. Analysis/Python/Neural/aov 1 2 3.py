...
if STIM_LOCKED:
    if THETA_BAND:
        x_title, y_title, col_title = "Block number", "Theta power", "Condition"
    elif ALPHA_BAND:
        x_title, y_title, col_title = "Repetition count", "Alpha power", None
    else:
        x_title, y_title, col_title = "Repetition count", "Beta power", None
...
if THETA_BAND:
    ...
    g.fig.suptitle(r"Stimulus-locked $\theta$ ~ block number x condition") 
elif ALPHA_BAND:
    (g.set_xticklabels(np.arange(1, 9))
      .set(ylim = (-.5, -.2)))
    plt.subplots_adjust(top=0.9)
    g.fig.suptitle(r"Stimulus-locked $\alpha$ ~ repetition count") 
else:
    (g.set_xticklabels(np.arange(1, 9))
      .set(ylim = (-.35, -.15)))
    plt.subplots_adjust(top=0.9)
    g.fig.suptitle(r"Stimulus-locked $\beta$ ~ repetition count") 
...
if STIM_LOCKED:
    if THETA_BAND:
        ...
        ax_list[0].set_yticks(np.arange(-.1, .11, .05))   
        ...
        ax_list[1].set_yticks(np.arange(-.1, .11, .05))   
        ...
        (g.set(ylim = (-.5, -.2),
               xlim = (.5, 8.5)))
        g.set_yticks(np.arange(-.5, -.19, 0.05))
        g.set_title(r"$\alpha$ power ~ fast timescale") 
        g.set_ylabel(r"$\alpha$ power")
        ...
        (g.set(ylim = (-.35, -.15),
               xlim = (.5, 8.5)))
        g.set_yticks(np.arange(-.35, -.145, 0.05))
        g.set_title(r"$\beta$ power ~ fast timescale") 
        g.set_ylabel(r"$\beta$ power")
        g.set_xlabel("Stimulus number")
