...
ax_list[0].set_ylim([-.6, -.1])  
ax_list[0].set_yticks(np.arange(-.6, -.09, .1))   
...
# general title
fig.suptitle(r"$\alpha$ power ~ fast timescale (Pz)", fontsize = 8) 
...
plt.savefig(fname = os.path.join(FIG, "Figure S1 B.tiff"), dpi = 800)
plt.savefig(fname = os.path.join(FIG, "Figure S1 B.pdf"), dpi = 800)
